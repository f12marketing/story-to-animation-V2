...
description: >
  ...
  for each character and background using the fal.ai nano-banana API.
...
Generate reference images with fal.ai nano-banana, host them on imgbb,
...
- **FAL_API_KEY** — from [fal.ai](https://fal.ai/dashboard/keys) dashboard → Keys
...
1. **Generate** — POST to `https://queue.fal.run/fal-ai/nano-banana` with prompt and aspect_ratio
2. **Poll** — GET `status_url` until `status: COMPLETED`; fetch result from `response_url`
3. **Download** — save PNG to `./characters/{id}.png` or `./backgrounds/{id}.png`
4. **Upload to imgbb** — POST base64-encoded PNG to `api.imgbb.com/1/upload`; get permanent URL (retries 3×)
...
imgbb upload is retried up to 3 times; if all attempts fail the image is marked
as failed (no silent fallback to temporary URLs).
